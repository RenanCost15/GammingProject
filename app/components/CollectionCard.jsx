...
export default function CollectionCard({ title, image, countLabel, count, items = [] }) {
  ...
  const visibleItems = isOpen ? items.slice(0, 8) : items.slice(0, 3);
        ...
        <div className="space-y-5 p-6">
          <div>
            <div className="flex items-start justify-between gap-4">
              ...
              <span className={`mt-1 flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-crimson/30 bg-crimson/10 text-crimson transition duration-500 ${isOpen ? 'rotate-180 bg-crimson text-white' : 'group-hover:-translate-y-1'}`}>
                <FiChevronDown />
              </span>
            </div>
          ...
          {items.length > 0 && (
            <div className={`transition-all duration-500 ${isOpen ? 'max-h-80 opacity-100' : 'max-h-32 opacity-95'}`}>
