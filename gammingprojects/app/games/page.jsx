    ...
    <div className="page-shell">
      <section className="container-pro">
        <div className="mb-8 grid gap-6 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
          ...
          <div className="glass-panel rounded-3xl p-5"><div className="grid grid-cols-2 gap-3 text-sm"><div className="rounded-2xl border border-white/10 bg-black/25 p-4"><FiDatabase className="mb-2 text-xl text-crimson" /><p className="font-black text-frost">{cacheStats.entries}</p><p className="text-smoke">{t('games.cacheQueries')}</p></div><div className="rounded-2xl border border-white/10 bg-black/25 p-4"><FiClock className="mb-2 text-xl text-crimson" /><p className="font-black text-frost">{formatCacheTime(cacheStats.lastUpdate)}</p><p className="text-smoke">{t('games.lastUpdate')}</p></div></div></div>
        </div>
        <div className="glass-panel mb-8 rounded-3xl p-5 sm:p-6">
          ...
          <div className="mt-4 rounded-[1.75rem] border border-white/10 bg-black/25 p-4">
            ...
            <div className="grid gap-3 lg:grid-cols-[1.2fr_1fr]"><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{datePresets.filter((preset) => preset.value !== 'custom').map((preset) => <button key={preset.value} type="button" onClick={() => setDatePreset(preset.value)} className={`rounded-2xl border px-3 py-3 text-left transition ${draftFilters.sDatePreset === preset.value ? 'border-crimson bg-crimson text-white shadow-neon' : 'border-white/10 bg-white/5 text-ash hover:border-crimson hover:text-frost'}`}><span className="block text-xs font-black uppercase tracking-wide">{preset.label}</span><span className="mt-1 block text-[11px] leading-4 opacity-80">{preset.hint}</span></button>)}</div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4"><button type="button" onClick={() => setDatePreset('custom')} className={`mb-3 w-full rounded-xl border px-3 py-2 text-xs font-black uppercase tracking-wide transition ${draftFilters.sDatePreset === 'custom' ? 'border-crimson bg-crimson text-white' : 'border-white/10 bg-black/25 text-ash hover:border-crimson hover:text-frost'}`}>{t('games.useCustomRange')}</button><div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1"><label className="space-y-2"><span className="text-xs font-black uppercase tracking-wide text-smoke">{t('games.startDate')}</span><input type="date" value={draftFilters.sDateFrom === '1900-01-01' ? '' : draftFilters.sDateFrom} min="1950-01-01" max="9999-12-31" onChange={(event) => setDraftFilters((prev) => ({ ...prev, sDatePreset: 'custom', sDateFrom: event.target.value || '1900-01-01', sPage: 1 }))} className="field-pro w-full" /></label><label className="space-y-2"><span className="text-xs font-black uppercase tracking-wide text-smoke">{t('games.endDate')}</span><input type="date" value={draftFilters.sDateTo === '9999-12-31' ? '' : draftFilters.sDateTo} min="1950-01-01" max="9999-12-31" onChange={(event) => setDraftFilters((prev) => ({ ...prev, sDatePreset: 'custom', sDateTo: event.target.value || '9999-12-31', sPage: 1 }))} className="field-pro w-full" /></label></div></div></div>
          </div>
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-[1fr_1fr_auto]"><input type="text" name="sMetacritic" placeholder={t('games.metacriticPlaceholder')} value={draftFilters.sMetacritic} onChange={updateFilter} className="field-pro" /><select name="sPageSize" value={draftFilters.sPageSize} onChange={updateFilter} className="field-pro">{[8,16,24,32].map((size) => <option key={size} value={size}>{size} {t('games.perPage')}</option>)}</select><div className="flex items-center justify-center rounded-2xl border border-white/10 bg-black/25 px-5 text-sm font-bold text-smoke">{t('games.page')} {draftFilters.sPage}</div></div>
          <div className="mt-5 flex flex-wrap gap-2">{quickGenres.map((genre) => <button key={genre.value || 'all'} onClick={() => setQuickGenre(genre.value)} className={`rounded-full border px-4 py-2 text-xs font-black uppercase tracking-wide transition ${draftFilters.sGenres === genre.value ? 'border-crimson bg-crimson text-white shadow-neon' : 'border-white/10 bg-white/5 text-ash hover:border-crimson hover:text-frost'}`}>{genre.label}</button>)}</div>
        ...
        <div className="grid grid-cols-1 gap-7 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{games.map((game) => { const cover = game.background_image || buildImageFallback(game.name); const genres = game.genres?.slice(0, 2).map((genre) => genre.name).join(' • '); const platforms = game.parent_platforms?.slice(0, 3).map((item) => item.platform.name).join(', '); return <article key={game.id} className="card-pro group"><div className="h-64 bg-cover bg-center transition duration-500 group-hover:scale-105" style={{ backgroundImage: `linear-gradient(to top, rgba(5,5,5,.95), rgba(5,5,5,.08)), url(${cover})` }} /><div className="space-y-4 p-6"><div><div className="mb-3 flex items-center justify-between gap-3"><span className="rounded-full bg-crimson/15 px-3 py-1 text-xs font-black uppercase text-ember">{genres || t('common.game')}</span><span className="flex items-center gap-1 text-sm font-black text-crimson"><FiStar /> {game.rating || '—'}</span></div><h2 className="line-clamp-2 min-h-16 text-2xl font-black text-frost">{game.name}</h2><p className="mt-2 text-sm font-semibold text-smoke">{t('games.released')}: <span className="text-ash">{game.released || t('common.notInformed')}</span></p><p className="mt-1 line-clamp-1 text-sm font-semibold text-smoke">{t('games.platforms')}: <span className="text-ash">{platforms || t('common.notInformed')}</span></p></div><Link href={`/games/${game.id}`} className="btn-primary w-full">{t('games.details')}</Link></div></article>; })}</div>
